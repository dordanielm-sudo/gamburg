...
`pm2 startup` אינו אופציונלי. בלעדיו כל אתחול של השרת - כולל שדרוג חבילה
ב-Cloudways - משאיר את השרת חי ואת האפליקציה כבויה.

### מנהרת Cloudflare

`ecosystem.config.cjs` מגדיר גם תהליך `cloudflared`, בתנאי שהבינארי קיים
בשורש הפרויקט. הוא מה שמחבר את הדומיין לשרת - בלעדיו הדפדפן מקבל
**שגיאה 1033 של Cloudflare**, גם כשהאפליקציה עצמה עובדת מצוין מקומית.

התצורה שלו יושבת ב-`~/.cloudflared/config.yml` יחד עם קובץ ההרשאות, שניהם
מחוץ ל-git. אם השרת מוחלף, צריך להעתיק את שניהם ואת הבינארי לפני הפריסה.

אין להפעיל את המנהרה ידנית ב-`pm2 start` מחוץ לקובץ הזה. `scripts/deploy.sh`
מסתיים ב-`pm2 save`, שכותב את רשימת התהליכים הרצים באותו רגע - כך שתהליך
שאינו בקובץ ונפל מסיבה כלשהי נמחק מהרשימה השמורה בפריסה הבאה, ואפילו
`pm2 resurrect` כבר לא מחזיר אותו.
