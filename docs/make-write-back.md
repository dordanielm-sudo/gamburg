...
```json
{
  ...
  "sql":     "השאילתה המוכנה להרצה, עם ? במקום ערכים — או null",
  "params":  ["הערכים, בסדר שבו ה-? מופיעים"],
...
ה-CRM בונה את השאילתה ושולח אותה מוכנה. הסנריו ב-Make לא צריך router, לא
מיפוי שמות שדות ולא שאילתות שנכתבות ביד:
...
**הערכים לעולם אינם בתוך טקסט השאילתה.** ה-`?` נשארים, והערכים נוסעים
ב-`params` ונקשרים על ידי הדרייבר. זה חשוב כאן יותר מבכל מקום אחר: הערך
הוא מה שמשתמש הקליד בטופס, והמחרוזת הזו מורצת כפי שהיא על מסד הייצור של
המשרד.

מכאן גם ש**כתובת הוובהוק היא סוד**. מי שיכול לשלוח אליה יכול להריץ את
