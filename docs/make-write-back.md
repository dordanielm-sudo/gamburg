...
| `deadline` | מועד | `case_number` + `page_name` + `source_ref`, כאשר `source_ref` הוא **שם השדה** בעדכנית שממנו נגזר המועד (למשל `תאריך דיון.`) |
...
וגם בצו שיקום כלכלי. בלי `page_name` הכתיבה לא יודעת לאיזה חוצץ לפנות —
ולתשעה שמות שדה שנבדקו יש התנגשות כזו בפועל, כולל `מועד העלאת הצו`.

זה נכון גם למועדים, לא רק לחוצצים: `case_deadlines` שומרת `page_name`
לצד `source_field_name` (מיגרציה 0041), ממולא מהסנכרון הנכנס. מועד שנוצר
לפני המיגרציה יחזיק `page_name = null` עד שהסנכרון הבא ימלא אותו; עד אז
הכתיבה שלו נופלת לחיפוש הישן שדורש ששם השדה יהיה ייחודי בכל החוצצים —
ומעדכנת בהצלחה כל עוד השם שלו אינו אחד מהמתנגשים.
...
מועד אינו רשומה נפרדת בעדכנית אלא **שדה תאריך על התיק** — אותו מנגנון
בדיוק כמו `case_field` (ראו שם: שש הטבלאות, ה-upsert, מיפוי הסוגים). ההבדל
היחיד: שם השדה מגיע ב-`source_ref` במקום ב-`field_name`, כי מבחינת ה-CRM
מועד הוא רשומה משלו ב-`case_deadlines`, לא שורה ב-`case_fields`.

`value_type` נשלח כ-`date` תמיד — מועד הוא שדה תאריך, אין צורך לנחש מצורת
הערך.
